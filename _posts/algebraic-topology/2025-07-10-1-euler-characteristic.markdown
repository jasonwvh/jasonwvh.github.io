...
One of the oldest and most fundamental topological invariants is the **Euler characteristic**, denoted by χ (chi). For a 2-dimensional surface, the formula is remarkably simple:
...
- **A single point:** V=1, E=0, F=0. χ = 1.
- **A line segment:** Two vertices, one edge. V=2, E=1, F=0. χ = 2 - 1 = 1.
...
- **A circle (triangulated):** Imagine a triangle. V=3, E=3, F=0 (we are only considering the boundary graph). χ = 3 - 3 = 0. What if we use a square? V=4, E=4, F=0. χ = 4 - 4 = 0. Any graph that is a single loop will have χ = 0.

Now let's look at surfaces:
- **A filled triangle:** V=3, E=3, F=1. χ = 3 - 3 + 1 = 1.
- **A filled square (made of two triangles):** V=4, E=5, F=2. χ = 4 - 5 + 2 = 1.
- **A hollow tetrahedron (a sphere):** V=4, E=6, F=4. χ = 4 - 6 + 4 = 2.
- **A hollow cube (also a sphere):** V=8, E=12, F=6 (if we treat the square faces as single entities, which is fine for the Euler characteristic). χ = 8 - 12 + 6 = 2.
...
Because any continuous deformation can be approximated by such subdivisions, the Euler characteristic is a **topological invariant**. This means the sphere (χ=2), the disk (χ=1), and the circle (χ=0) are fundamentally different topological objects.

### A More Complex Example: The Torus

Let's compute χ for a torus (donut). We can build a torus by taking a rectangle and gluing opposite sides.
...
So, for the torus, χ = V - E + F = 1 - 2 + 1 = 0.

This is fascinating! A circle and a torus both have χ=0. This suggests the Euler characteristic, while powerful, can't tell the whole story. It's a single number, after all. To distinguish these shapes, we need a more powerful tool that can count holes in different dimensions. That tool is **homology**, the subject of our next post.
